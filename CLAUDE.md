...
framework discovers and wires. `redsun` documents the rules that belong to the
framework; this file records only what is specific to the bundle.
...
The entry point is
`[project.entry-points."redsun.plugins"] redsun-mimir = "redsun.yaml"`, which
redsun's plugin discovery reads. `mimir <sim|uc2|motor|light|acquisition>`
...
`cmd.exe` has no `$(...)`, so on Windows use PowerShell and write the mypy
arguments as `@(uv run qtpy mypy-args)`. Prefer PowerShell over `cmd.exe` for
Claude Code sessions on this repo.

### mypy

Always run mypy through the qtpy shim. It pins the Qt binding mypy resolves
against, and it is what CI runs; a bare `mypy src/` diverges once both pyqt6
and pyside6 are present.

mypy is `strict = true` with `warn_unreachable` and `files = ["src", "tests"]`,
so tests are type-checked as strictly as the package, same as redsun. Only
`import-untyped`, `import-not-found` and `no-untyped-call` are disabled. Do not
widen that list to silence a real error.

### pytest

`asyncio_mode = "auto"` is set, so do not decorate async tests with
`@pytest.mark.asyncio`.

Skip markers are public names (`needs_opengl`, `needs_mm_adapters`). Each
marker is imported by the modules that use it, so an underscore only makes that
import look like a mistake.
...
`conftest.py` and never build a `QApplication` yourself.

`offscreen` has no OpenGL context and napari's `QtViewer` needs a real one, so
anything constructing an `ImageView` carries `needs_opengl` from `conftest.py`
and is skipped unless `MIMIR_TEST_OPENGL` is set. CI provides the context with
`pyvista/setup-headless-display-action` and sets that variable, which is why no
workflow pins `QT_QPA_PLATFORM` for the test run.

### Micro-Manager adapters

The adapters are downloaded, not pip-installed, and their device interface
version must match the installed `pymmcore` - `mmcore list` marks a mismatched
install `(incompatible)`. Install only the test adapters (`--test-adapters`):
every mmcore device here is `adapter="DemoCamera"`, and that is what CI
installs via `pymmcore-plus/setup-mm-test-adapters`. Mark a test that loads a
real device with `needs_mm_adapters` from `conftest.py`, so a machine without
the adapters skips instead of failing inside `loadDevice`.

## Architecture invariants

Every device, presenter and view class intended for users must appear in
...
redsun no matter how correct it is, and a manifest entry that does not resolve
breaks plugin discovery for the whole bundle. Adding a component -> same-commit
manifest entry.

Devices are `ophyd-async` devices. Hardware access is async; no threads for
I/O. `DeviceMap` comes from `ophyd_async.core`.

Presenter constructors must lead with exactly `(name, devices)` and view
constructors with `(name)`. The framework validates the positional shape at
discovery and the protocol via `isinstance` on the built instance. Do not
inherit `PPresenter`/`PView`: property descriptors shadow instance attributes
at runtime. Data members exposed to the framework are read-only properties.

Signals are named `sig_snake_case`, never `sigCamelCase`. Signals sharing a
name across components are disambiguated by owner:
...
(`write`/`release`/`close`), and `BaseStorage` implements `SinkFactory` over
both. Producers only ever hold a `FrameSink`. Do not reimplement queueing,
draining or frame counting here.

A detector sharing storage must not open the backend at prepare: a sibling's
`register` races the open and raises `StoreStateError`. Rely on the drain's
lazy open instead. Opening at prepare is only legal when a detector
exclusively owns its storage group.

A plan collecting a `StandardDetector` must pre-declare the stream with
`bps.declare_stream(det, name=..., collect=True)` before `bps.collect`.

Device build failures are logged and skipped; presenter and view build failures
abort the app. A device that cannot reach hardware must fail at build, not
half-initialise.

`youseetoo/` talks to real serial hardware and cannot be covered in CI. Keep it
behind mocks in tests and test it manually (see README warning).
...
- The example containers in `configurations/` are shipped artifacts, so they
  must be smoke-tested with mock devices (build the container, assert the
  components come up) rather than left to manual runs.
...
- All imports live at the top of the module, in tests too. No function-level
  imports; runtime-unneeded imports go under `if TYPE_CHECKING:`.
