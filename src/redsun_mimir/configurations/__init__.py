...
Each module exposes a ``build_*`` factory returning the container unbuilt, so
it can be inspected without entering the Qt event loop, and a ``run_*`` entry
point for the ``mimir`` CLI.
"""
